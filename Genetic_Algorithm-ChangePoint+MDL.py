...
def initial_pop(n_chrom, size, p_zero, steps):
    ...
    for chromosome in range(n_chrom):    
        chromo = np.zeros(size) # pseudo chromosome with all zeros

        i = 0
        chromo[0] = 1
        i = steps

        while i < len(chromo): # stop before the last t_steps
            chromo[i]= np.random.choice( np.arange(0, 2), p=[p_zero,1 - p_zero] )                
            if chromo[i] == 1:
                i = i + steps # jump t steps if 1 is picked
            else: i = i + 1
        chromo[(len(chromo) - steps + 1) : ] = 0 # replace the last bit to 0
        
        population.append(np.array(chromo))
